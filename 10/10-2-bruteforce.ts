...
const machines = inputLines.map(line => {
    ...
    return {
        ...
        buttons,
        joltages,
        maxDepthsAssumed: tk.sum(joltagesStr.split(',').map(js => +js)),
    } as Machine;
});
...
const solutionDepths = machines.slice(0,3).map(machine => bfs(machine, machine.maxDepthsAssumed))
